        ...
        legendControl.onAdd = function() {
            ...
            div.innerHTML = `
                <div style="position: relative; z-index: 10;">
                    <h4 style="margin: 0 0 12px 0; font-weight: bold; background: linear-gradient(135deg, #002147, #FF6600, #002147); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
                        ACC Teams
                    </h4>
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                        {rank && (
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-lg" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}>
                                {rank}
                            </div>
                        )}
                        ...
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center justify-between text-xs">
                                    <span className="text-gray-500">{article.source?.name || article.source || 'ACC Network'}</span>
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <div className="space-y-2 text-xs">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium text-gray-800">ACC</span>
                                                </div>
                                                <div className="flex justify-between">
...
// Sidebar Team Navigator Component - 3 teams stacked for ACC
const SidebarTeamNavigator = ({ teams, onTeamClick }) => {
    return (
        <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-6 mb-6">
            ...
            <div className="relative z-10">
                <h3 className="text-lg font-bold mb-4" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                    Team Navigator
                </h3>
                <p className="text-sm text-gray-600 mb-4">Click teams to navigate the map</p>
                
                {/* 3 Teams stacked vertically */}
                <div className="space-y-3">
                    {teams.slice(0, 15).map((team, index) => {
                        if (index % 5 !== 0) return null; // Show every 5th team for spacing
...
const ACC = () => {
  ...
  const loadACCData = async () => {
    ...
    try {
      console.log("Starting ACC data fetch process...");
      ...
      const accTeams = allTeams.filter(team => 
        team.conference === "ACC" || 
        team.conference === "Atlantic Coast Conference" ||
        team.conference?.includes("ACC") ||
        team.conference?.includes("Atlantic Coast")
      );
      console.log("ACC teams found:", accTeams.map(t => t.school));
      
      const teamsWithLocations = accTeams.filter(team =>
          team.location && team.location.latitude && team.location.longitude
      );

      if (teamsWithLocations.length === 0) {
          console.warn("No ACC teams with location data found, using all ACC teams");
          setTeams(accTeams);
      } else {
          console.log(`Found ${teamsWithLocations.length} ACC teams with locations`);
      ...
      if (teamsWithLocations.length > 0) {
          ...
          // Use default ACC region center (Atlantic Coast)
      ...
      try {
          ...
          if (talentData && talentData.length > 0) {
              ...
              // Filter for ACC teams and sort by talent score (highest to lowest)
              const accTalent = talentData
                  .filter(talentTeam => {
                      const teamName = talentTeam.team || talentTeam.school;
                      return accTeams.some(t => 
                          t.school === teamName || 
              ...
              setTeamTalent(accTalent);
              console.log("ACC talent data:", accTalent);
          } else {
      ...
      try {
          ...
          if (recruitsData && recruitsData.length > 0) {
              // Filter for ACC commits and sort by rating
              const accRecruits = recruitsData
                  .filter(recruit => 
                      recruit.committedTo && 
                      accTeams.some(t => 
              ...
              setRecruits(accRecruits);
              console.log("ACC recruits:", accRecruits);
          } else {
              ...
              if (teamRecruitingData && teamRecruitingData.length > 0) {
                  const accTeamRecruits = teamRecruitingData
                      .filter(recruit => 
                          accTeams.some(t => 
      ...
      console.log("Fetching records for ACC teams...");
      ...
      try {
          ...
          // Map ACC teams with their records
          const standingsData = accTeams.map(team => {
      ...
      // Load recent games for ACC teams
      try {
          const currentYear = 2024;
          const recentGames = await gameService.getGames(currentYear, null, 'regular', null, null, null, 'ACC');
          const accGames = recentGames.filter(game => 
              accTeams.some(t => t.school === game.home_team || t.school === game.away_team)
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            <div className="relative">
              <div className="animate-spin rounded-full h-16 w-16 border-4 border-transparent mx-auto" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
            ...
            <div className="mt-6 space-y-2">
              <p className="text-xl font-bold" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Loading ACC Conference...
              </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl max-w-md mx-auto">
              <h3 className="font-bold">Error Loading ACC Data</h3>
              <p className="text-sm mt-2">{errorMessage}</p>
              <button 
                onClick={loadACCData}
                className="mt-4 px-4 py-2 text-white rounded-lg hover:opacity-90 transition-opacity"
                style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}
              >
                Try Again
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <style jsx>{`
        .gradient-bg {
          background: linear-gradient(135deg, #002147, #FF6600, #002147);
        }
        .gradient-text {
          background: linear-gradient(135deg, #002147, #FF6600, #002147);
      ...
      {/* Floating Orbs Background - ACC themed */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-64 h-64 rounded-full opacity-5 blur-3xl animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
        <div className="absolute top-60 right-20 w-48 h-48 rounded-full opacity-3 blur-2xl animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', animationDelay: '1s' }}></div>
        <div className="absolute bottom-40 left-1/4 w-80 h-80 rounded-full opacity-4 blur-3xl animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', animationDelay: '2s' }}></div>
        <div className="absolute bottom-20 right-1/3 w-56 h-56 rounded-full opacity-3 blur-2xl animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', animationDelay: '3s' }}></div>
      </div>
        ...
        <div className="text-center mb-20">
          <div className="flex items-center justify-center mb-8 relative">
            ...
            <div className="relative">
              ...
              <div className="relative w-16 h-16 rounded-full bg-white/40 backdrop-blur-2xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_10px_30px_rgba(0,0,0,0.1)] flex items-center justify-center">
                ...
                <i className="fas fa-trophy text-3xl relative z-10 drop-shadow-lg" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}></i>
              </div>
              {/* Floating particles */}
              <div className="absolute -top-2 -right-2 w-3 h-3 rounded-full opacity-60 animate-ping" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
              <div className="absolute -bottom-2 -left-2 w-2 h-2 rounded-full opacity-40 animate-ping animation-delay-500" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
            </div>
          </div>
          ...
          <div className="relative mb-8">
            <h1 className="text-6xl md:text-7xl font-black mb-6 relative">
              <span className="drop-shadow-2xl" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>ACC</span>
              <br />
              <span className="drop-shadow-2xl" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Conference</span>
              {/* Animated underline */}
              <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-32 h-1 rounded-full opacity-60 animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
            </h1>
          </div>
          ...
          <div className="inline-flex items-center space-x-4 px-8 py-4 rounded-full bg-white/20 backdrop-blur-xl border border-white/30 shadow-[inset_0_2px_10px_rgba(255,255,255,0.2),0_15px_35px_rgba(0,0,0,0.1)]">
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 rounded-full animate-pulse" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
              <span className="text-lg font-bold" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{teams.length} Teams</span>
            </div>
              ...
              <div className="flex flex-wrap items-center gap-4 justify-center">
                {categories.map(category => (
                  ...
                  >
                    ...
                    {selectedCategory === category && (
                      <div className="absolute inset-0 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)]" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)' }}></div>
                    )}
          ...
          <div className="lg:col-span-2">
            ...
            {selectedCategory === 'Talent Rankings' && (
              <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-8">
                ...
                <div className="relative z-10">
                  <h2 className="text-2xl font-bold mb-6" style={{ background: 'linear-gradient(135deg, #002147, #FF6600, #002147)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                  ...
                  {standings && standings.length > 0 ? (
                      ...
                      <table className="w-full border-collapse">
                        ...
                        <tbody>
                          {standings.map((team, index) => {
                            ...
                            const championGradient = 'linear-gradient(135deg, #002147, #FF6600, #002147)'; // ACC Conference champion
                            ...
                            if (isChampion) {
                              trophyIconStyle = { 
                                ...
                                WebkitTextFillColor: 'transparent', 
                                backgroundClip: 'text',
                                filter: 'drop-shadow(0 2px 4px rgba(0, 33, 71, 0.4))'
                              };
                              showTrophy = true;
                            }
                    ...
                    <div className="text-center py-8">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg hover:from-orange-600 hover:to-orange-700 transition-all duration-300"
                      >
                        Refresh Data
                      </button>
                    </div>
                ...
                <div className="relative z-10">
                  ...
                  {rankings && rankings.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">No ACC teams in current AP Top 25</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-8 text-center">
              ...
              <div className="relative z-10">
                <img 
                  src="/photos/ACC.png" 
                  alt="Atlantic Coast Conference"
                  className="w-32 h-32 mx-auto mb-4 object-contain"
                  onError={(e) => { e.target.style.display = 'none'; }}
                />
                <h3 className="text-xl font-bold text-gray-800 mb-2">Atlantic Coast Conference</h3>
                <p className="text-gray-600 text-sm">Est. 1953 • 15 Teams</p>
              </div>
            </div>
