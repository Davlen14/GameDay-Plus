        ...
        legendControl.onAdd = function() {
            ...
            div.innerHTML = `
                <div style="position: relative; z-index: 10;">
                    <h4 style="margin: 0 0 12px 0; font-weight: bold; background: linear-gradient(135deg, #DC143C, #B22222, #DC143C); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
                        Big 12 Teams
                    </h4>
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                        {rank && (
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-lg" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}>
                                {rank}
                            </div>
                        )}
                        ...
                        <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center justify-between text-xs">
                                    <span className="text-gray-500">{article.source?.name || article.source || 'Big 12 Network'}</span>
...
const ConferenceMap = ({ teams, onTeamClick, mapCenter, mapZoom }) => {
    // Custom marker icon for Big 12 teams
                ...
                >
                    ...
                    {teams.map(team => (
                        team.location && (
                            <Marker
                                key={team.id}
                                position={[team.location.latitude || 39.0, team.location.longitude || -98.0]}
                                        ...
                                        <div className="relative z-10">
                                            ...
                                            <div className="space-y-2 text-xs">
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium text-gray-800">Big 12</span>
                                                </div>
                                                <div className="flex justify-between">
...
const Big12 = () => {
  ...
  const [mapCenter, setMapCenter] = useState([39.0, -98.0]); // Central US for Big 12
  ...
  const loadBig12Data = async () => {
    setIsLoading(true);
    setErrorMessage(null);

    try {
      console.log("Starting Big 12 data fetch process...");
      ...
      const big12Teams = allTeams.filter(team => 
        team.conference === "Big 12" || 
        team.conference === "Big XII" ||
        team.conference?.includes("Big 12") ||
        team.conference?.includes("Big XII")
      );
      console.log("Big 12 teams found:", big12Teams.map(t => t.school));
      
      const teamsWithLocations = big12Teams.filter(team =>
          team.location && team.location.latitude && team.location.longitude
      );

      if (teamsWithLocations.length === 0) {
          console.warn("No Big 12 teams with location data found, using all Big 12 teams");
          setTeams(big12Teams);
      } else {
          console.log(`Found ${teamsWithLocations.length} Big 12 teams with locations`);
      ...
      if (teamsWithLocations.length > 0) {
          ...
          // Use default Big 12 region center
          setMapCenter([39.0, -98.0]);
      }
      ...
      try {
          ...
          if (talentData && talentData.length > 0) {
              ...
              // Filter for Big 12 teams and sort by talent score (highest to lowest)
              const big12Talent = talentData
                  .filter(talentTeam => {
                      const teamName = talentTeam.team || talentTeam.school;
                      return big12Teams.some(t => 
                          t.school === teamName || 
              ...
              setTeamTalent(big12Talent);
              console.log("Big 12 talent data:", big12Talent);
          } else {
      ...
      try {
          ...
          if (recruitsData && recruitsData.length > 0) {
              // Filter for Big 12 commits and sort by rating
              const big12Recruits = recruitsData
                  .filter(recruit => 
                      recruit.committedTo && 
                      big12Teams.some(t => 
              ...
              setRecruits(big12Recruits);
              console.log("Big 12 recruits:", big12Recruits);
          } else {
              ...
              if (teamRecruitingData && teamRecruitingData.length > 0) {
                  const big12TeamRecruits = teamRecruitingData
                      .filter(recruit => 
                          big12Teams.some(t => 
      ...
      console.log("Fetching records for Big 12 teams...");
      ...
      try {
          ...
          // Map Big 12 teams with their records
          const standingsData = big12Teams.map(team => {
      ...
      // Load recent games for Big 12 teams
      try {
          const currentYear = 2024;
          const recentGames = await gameService.getGames(currentYear, null, 'regular', null, null, null, 'Big 12');
          const big12Games = recentGames.filter(game => 
              big12Teams.some(t => t.school === game.home_team || t.school === game.away_team)
      ...
      console.error('Error loading Big 12 data:', error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            <div className="relative">
              <div className="animate-spin rounded-full h-16 w-16 border-4 border-transparent mx-auto" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
            ...
            <div className="mt-6 space-y-2">
              <p className="text-xl font-bold" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Loading Big 12 Conference...
              </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center">
            <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl max-w-md mx-auto">
              <h3 className="font-bold">Error Loading Big 12 Data</h3>
              <p className="text-sm mt-2">{errorMessage}</p>
              <button 
                onClick={loadBig12Data}
                className="mt-4 px-4 py-2 text-white rounded-lg hover:opacity-90 transition-opacity"
                style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}
              >
                Try Again
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <style jsx>{`
        .gradient-bg {
          background: linear-gradient(135deg, #DC143C, #B22222, #DC143C);
        }
        .gradient-text {
          background: linear-gradient(135deg, #DC143C, #B22222, #DC143C);
      ...
      {/* Floating Orbs Background - Big 12 themed */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 left-10 w-64 h-64 rounded-full opacity-5 blur-3xl animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
        <div className="absolute top-60 right-20 w-48 h-48 rounded-full opacity-3 blur-2xl animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', animationDelay: '1s' }}></div>
        <div className="absolute bottom-40 left-1/4 w-80 h-80 rounded-full opacity-4 blur-3xl animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', animationDelay: '2s' }}></div>
        <div className="absolute bottom-20 right-1/3 w-56 h-56 rounded-full opacity-3 blur-2xl animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', animationDelay: '3s' }}></div>
      </div>
        ...
        <div className="text-center mb-20">
          <div className="flex items-center justify-center mb-8 relative">
            ...
            <div className="relative">
              ...
              <div className="relative w-16 h-16 rounded-full bg-white/40 backdrop-blur-2xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_10px_30px_rgba(0,0,0,0.1)] flex items-center justify-center">
                ...
                <i className="fas fa-trophy text-3xl relative z-10 drop-shadow-lg" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}></i>
              </div>
              {/* Floating particles */}
              <div className="absolute -top-2 -right-2 w-3 h-3 rounded-full opacity-60 animate-ping" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
              <div className="absolute -bottom-2 -left-2 w-2 h-2 rounded-full opacity-40 animate-ping animation-delay-500" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
            </div>
          </div>
          ...
          <div className="relative mb-8">
            <h1 className="text-6xl md:text-7xl font-black mb-6 relative">
              <span className="drop-shadow-2xl" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Big 12</span>
              <br />
              <span className="drop-shadow-2xl" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Conference</span>
              {/* Animated underline */}
              <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-32 h-1 rounded-full opacity-60 animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
            </h1>
          </div>
          ...
          <div className="inline-flex items-center space-x-4 px-8 py-4 rounded-full bg-white/20 backdrop-blur-xl border border-white/30 shadow-[inset_0_2px_10px_rgba(255,255,255,0.2),0_15px_35px_rgba(0,0,0,0.1)]">
            <div className="flex items-center space-x-2">
              <div className="w-3 h-3 rounded-full animate-pulse" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
              <span className="text-lg font-bold" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>{teams.length} Teams</span>
            </div>
              ...
              <div className="flex flex-wrap items-center gap-4 justify-center">
                {categories.map(category => (
                  ...
                  >
                    ...
                    {selectedCategory === category && (
                      <div className="absolute inset-0 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.3)]" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)' }}></div>
                    )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            {selectedCategory === 'Map' && (
              <div>
                <div className="mb-6">
                  <h2 className="text-2xl font-bold mb-4" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                ...
                <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-6">
                  ...
                  <div className="relative z-10">
                    <h3 className="text-lg font-bold mb-4" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                      Quick Navigation
                    </h3>
            ...
            {selectedCategory === 'Talent Rankings' && (
              <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-8">
                ...
                <div className="relative z-10">
                  <h2 className="text-2xl font-bold mb-6" style={{ background: 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                      ...
                      <table className="w-full border-collapse">
                        ...
                        <tbody>
                          {standings.map((team, index) => {
                            ...
                            const championGradient = 'linear-gradient(135deg, #DC143C, #B22222, #DC143C)'; // Big 12 Conference champion
                ...
                <div className="relative z-10">
                  ...
                  {rankings && rankings.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">No Big 12 teams in current AP Top 25</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            <div className="relative bg-white/40 backdrop-blur-2xl rounded-3xl border border-white/50 shadow-[inset_0_2px_10px_rgba(255,255,255,0.3),0_20px_40px_rgba(0,0,0,0.1)] p-8 text-center">
              ...
              <div className="relative z-10">
                <img 
                  src="/photos/Big 12.png" 
                  alt="Big 12 Conference"
                  className="w-32 h-32 mx-auto mb-4 object-contain"
                  onError={(e) => { e.target.style.display = 'none'; }}
                />
                <h3 className="text-xl font-bold text-gray-800 mb-2">Big 12 Conference</h3>
                <p className="text-gray-600 text-sm">Est. 1996 • 16 Teams</p>
              </div>
            </div>
